...
class BaseDescent:
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        raise ValueError("Impossible Loss Function")
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ind = np.random.randint(0, x.shape[0], self.batch_size, dtype=int)
        batch_x = x[ind]
        batch_y = y[ind]
        return super().calc_gradient(batch_x, batch_y)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff = -(self.lr() * m / (np.sqrt(v) + self.eps))
        self.w += diff
